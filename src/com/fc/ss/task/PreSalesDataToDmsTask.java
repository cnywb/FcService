 ...
 * 
 */
public class PreSalesDataToDmsTask implements Job{
	private static Logger log = Logger.getLogger(PreSalesDataToDmsTask.class);
	...
	private void initDmsLogMAP(String importDate) {
		...
		for (int i = 0; i <6; i++) {
			if (!(PreSalesDataToDmsTask.toLogBeanMap.containsKey(aryReadType[i]))) {
				DmsToLogBean tologBean = PreSalesDataToDmsTask.createDmsToLogBean(aryReadType[i],
						importDate);
				PreSalesDataToDmsTask.toLogBeanMap.put(aryReadType[i], tologBean);
			}
		}
	}
